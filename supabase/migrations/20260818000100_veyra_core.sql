...
create or replace function public.seed_default_habits(target_user uuid)
returns void language plpgsql security definer set search_path = public as $$
begin
  insert into public.habits (user_id, name, custom)
  select target_user, v.name, false from (values ('Workout'),('Sleep routine'),('Hydration'),('Personal grooming'),('Movement'),('Study/work focus')) v(name)
  where not exists (select 1 from public.habits h where h.user_id = target_user);
end;
$$;
...
insert into public.habits (user_id, name, custom)
select p.id, v.name, false from public.profiles p
