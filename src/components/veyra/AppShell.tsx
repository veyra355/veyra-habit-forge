...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (hydrated && !authLoading && !state.user) navigate({ to: "/auth", replace: true });
  }, [hydrated, authLoading, state.user, navigate]);
  ...
  const xpProgress = Math.max(
    ...
    Math.min(
      100,
      ((state.totalXp - xpFromLevelStart) / Math.max(1, xpToNextLevel - xpFromLevelStart)) * 100,
    ),
  );
  return (
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center justify-between gap-2 text-[11px] font-semibold uppercase tracking-wider">
              <span className="truncate">
                {state.currentRank} · Level {state.currentLevel}
              </span>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-border/70 bg-background/95 backdrop-blur-xl lg:hidden">
        <div className="safe-bottom flex items-stretch justify-between px-1">
          ...
          <Sheet open={moreOpen} onOpenChange={setMoreOpen}>
            ...
            <SheetContent
              side="bottom"
              className="rounded-t-3xl border-border pb-[calc(1.5rem+env(safe-area-inset-bottom))]"
            >
  ...
  return (
    <div className="mb-5 grid grid-cols-[minmax(0,1fr)] items-end gap-3 sm:mb-6 sm:flex sm:flex-wrap sm:justify-between">
      <div className="min-w-0">
        <h1 className="display-italic text-[clamp(1.65rem,7vw,2.35rem)]">{title}</h1>
        {subtitle && (
