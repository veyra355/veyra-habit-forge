...
import { toQuestCardModel } from "@/lib/quest-display";
...
function GameModePage() {
  ...
  const { loading, daily, weekly, missions, achievements, completeQuest, completing, levelUpTo, clearLevelUp } = useGame();
  ...
  const boards = [
    { eyebrow: "Daily quests", title: "Today's mission board", items: daily, reset: "Resets daily" },
    { eyebrow: "Weekly quests", title: "This week's objectives", items: weekly, reset: "Resets weekly" },
    { eyebrow: "Missions", title: "Long-term missions", items: missions, reset: "One-time" },
  ];
  ...
  return (
    <>
      ...
      {boards.map((board) => (
        <section key={board.eyebrow} className="mt-7">
          <SectionTitle eyebrow={board.eyebrow} title={board.title} trailing={board.reset} />
          {loading ? <div className="mt-4 rounded-3xl border border-border bg-card p-6 text-sm text-muted-foreground">Loading your mission board…</div> : board.items.length === 0 ? <div className="mt-4 rounded-3xl border border-dashed border-border bg-card p-6 text-sm text-muted-foreground">No active missions right now. Check back soon.</div> : <div className="mt-4 grid gap-3 lg:grid-cols-3">{board.items.map((quest) => <QuestCard key={quest.id} quest={toQuestCardModel(quest)} onComplete={() => completeQuest(quest)} />)}</div>}
        </section>
      ))}

      <section className="mt-7 grid gap-4 lg:grid-cols-[1.1fr_.9fr]">
        <div className="rounded-3xl border border-border bg-card p-5 sm:p-6"><SectionTitle eyebrow="Progression" title="Rank ladder" /><div className="mt-5 grid gap-3 sm:grid-cols-2">{RANKS.map((rank) => { const Icon = rank.icon; const unlocked = state.currentLevel >= rank.level; return <div key={rank.name} className={`rounded-2xl border p-4 ${unlocked ? "border-primary/40 bg-primary/5" : "border-border opacity-60"}`}><div className="flex items-center gap-3"><div className="flex size-10 items-center justify-center rounded-xl bg-secondary">{unlocked ? <Icon className="size-5 text-primary" /> : <Lock className="size-4" />}</div><div><p className="font-bold">{rank.name}</p><p className="text-xs text-muted-foreground">Level {rank.level} · {rank.desc}</p></div></div>{unlocked && <div className="mt-3 flex items-center gap-1 text-xs font-semibold text-primary"><Check className="size-4" /> Unlocked</div>}</div>; })}</div></div>
        <div className="rounded-3xl border border-border bg-card p-5 sm:p-6"><SectionTitle eyebrow="Collection" title="Achievements" /><div className="mt-5 grid gap-2 sm:grid-cols-2">{achievements.map((item) => <div key={item.id} className={`rounded-2xl border p-3 ${item.unlocked ? "border-primary/40 bg-primary/5" : "border-border"}`}><div className="flex items-center gap-2"><Trophy className={`size-4 ${item.unlocked ? "text-primary" : "text-muted-foreground"}`} /><span className="text-sm font-semibold">{item.title}</span></div><p className="mt-1 text-xs text-muted-foreground">{item.description}</p>{!item.unlocked && <p className="mt-1 text-[11px] text-muted-foreground">Goal: {item.threshold}</p>}</div>)}</div></div>
      </section>

      <section className="mt-7 rounded-3xl border border-border bg-gradient-to-r from-card via-muted/40 to-card p-5 sm:p-6"><SectionTitle eyebrow="Game loop" title="Your Veyra progression" /><div className="mt-5 grid gap-3 sm:grid-cols-4"><LoopStep number="01" title="Choose" detail="Pick a quest" /><LoopStep number="02" title="Complete" detail="Do the real action" /><LoopStep number="03" title="Progress" detail="Build XP & streaks" /><LoopStep number="04" title="Unlock" detail="Reach new ranks" /></div></section>
      <section className="mt-7 grid gap-4 sm:grid-cols-3"><QuickCard icon={Swords} title="Workout" detail="Train and earn progress" href="/workout" /><QuickCard icon={Target} title="Habits" detail="Clear today's habits" href="/habits" /><QuickCard icon={Sparkles} title="AI Coach" detail="Get your next mission" href="/coach" /></section>
      <div className="mt-7 flex items-center gap-3 rounded-3xl border border-border bg-muted/30 p-5"><Users className="size-5 shrink-0 text-primary" /><div><p className="font-semibold">Social-ready foundation</p><p className="text-sm text-muted-foreground">Future leaderboards, teams and challenges can build on this quest system without using body or appearance comparisons.</p></div></div>
