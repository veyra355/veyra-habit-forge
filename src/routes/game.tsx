...
export const Route = createFileRoute("/game")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Turn your Veyra routine into quests, XP, ranks and achievements.",
      },
    ],
  }),
  component: () => (
...
function GameModePage() {
  ...
  const levelProgress = Math.max(
    0,
    Math.min(100, ((state.totalXp - previousLevel) / range) * 100),
  );
  const questBoards = [
    {
      eyebrow: "Daily quests",
      title: "Today's mission board",
      trailing: "Resets daily",
      items: daily,
    },
    {
      eyebrow: "Weekly quests",
      title: "This week's objectives",
      trailing: "Resets weekly",
      items: weekly,
    },
    {
      eyebrow: "Missions",
  ...
  return (
    <>
      {levelUpTo !== null && (
        <LevelUpAnimation level={levelUpTo} onComplete={clearLevelUp} />
      )}
      <PageHeader
        title="Game Mode"
        ...
        action={
          <Badge className="rounded-full px-3 py-1.5">
            <Gamepad2 className="mr-1.5 size-4" /> GAME ON
          </Badge>
        }
      />
      ...
      <section className="relative overflow-hidden rounded-[2rem] border border-primary/30 bg-gradient-to-br from-primary/15 via-card to-purple-500/10 p-5 shadow-lg sm:p-7">
        ...
        <div className="relative grid gap-6 lg:grid-cols-[1.25fr_.75fr] lg:items-center">
          <div>
            <div className="flex items-center gap-2 text-xs font-black uppercase tracking-[.2em] text-primary">
              <Zap className="size-4" /> Player HUD
            </div>
            ...
            <p className="mt-2 max-w-xl text-sm text-muted-foreground">
              {Math.max(0, nextLevel - state.totalXp)} XP to your next level. Progress comes from
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              <Button asChild className="rounded-full">
                <Link to="/workout">
                  <Swords className="mr-2 size-4" /> Start Quest
                </Link>
              </Button>
              <Button asChild variant="outline" className="rounded-full">
                <Link to="/habits">Daily Habits</Link>
              </Button>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <HudStat icon={Flame} label="Current streak" value={`${state.currentStreak}d`} />
            <HudStat icon={Trophy} label="Best streak" value={`${state.longestStreak}d`} />
            <HudStat icon={Swords} label="Sessions" value={`${state.sessions.length}`} />
            <HudStat icon={Zap} label="Total XP" value={`${state.totalXp}`} />
          </div>
        </div>
      </section>

      {questBoards.map((board) => (
        <section key={board.eyebrow} className="mt-7">
          <SectionTitle
            eyebrow={board.eyebrow}
            title={board.title}
            trailing={board.trailing}
          />
          {loading ? (
            <div className="mt-4 rounded-3xl border border-border bg-card p-6 text-sm text-muted-foreground">
              Loading your mission board…
            </div>
          ) : board.items.length === 0 ? (
            <div className="mt-4 rounded-3xl border border-dashed border-border bg-card p-6 text-sm text-muted-foreground">
              No active missions right now. Check back soon.
            </div>
          ) : (
            <div className="mt-4 grid gap-3 lg:grid-cols-3">
              {board.items.map((quest) => (
                <QuestCard
                  key={quest.id}
                  quest={toQuestCardModel(quest)}
        ...
        <div className="rounded-3xl border border-border bg-card p-5 sm:p-6">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            {RANKS.map((rank) => {
              ...
              const unlocked = state.currentLevel >= rank.level;
              return (
                <div
                  key={rank.name}
                  className={`rounded-2xl border p-4 ${unlocked ? "border-primary/40 bg-primary/5" : "border-border opacity-60"}`}
                >
        ...
        <div className="rounded-3xl border border-border bg-card p-5 sm:p-6">
          ...
          <div className="mt-5 grid gap-2 sm:grid-cols-2">
            {achievements.map((item) => (
              <div
                key={item.id}
                className={`rounded-2xl border p-3 ${item.unlocked ? "border-primary/40 bg-primary/5" : "border-border"}`}
              >
                <div className="flex items-center gap-2">
                  <Trophy
                    className={`size-4 ${item.unlocked ? "text-primary" : "text-muted-foreground"}`}
                  />
      ...
      <section className="mt-7 rounded-3xl border border-border bg-gradient-to-r from-card via-muted/40 to-card p-5 sm:p-6">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-4">
          <LoopStep number="01" title="Choose" detail="Pick a quest" />
          <LoopStep number="02" title="Complete" detail="Do the real action" />
          <LoopStep number="03" title="Progress" detail="Build XP & streaks" />
          <LoopStep number="04" title="Unlock" detail="Reach new ranks" />
        </div>
      </section>
      ...
      <div className="mt-7 flex items-center gap-3 rounded-3xl border border-border bg-muted/30 p-5">
        <Users className="size-5 shrink-0 text-primary" />
        <div>
          <p className="font-semibold">Social-ready foundation</p>
          <p className="text-sm text-muted-foreground">
            Future leaderboards, teams and challenges can build on this quest system without using
            body or appearance comparisons.
          </p>
        </div>
      </div>
      <SafetyNote>
        Game Mode is a motivation layer for general wellness. XP, ranks and streaks are not a
        measure of health, fitness level or personal worth. Missing a day does not mean you failed.
      </SafetyNote>
    </>
  );
}
...
function QuickCard({
  ...
  title: string;
  detail: string;
  href: "/workout" | "/habits" | "/coach";
}) {
  return (
    <Button asChild variant="outline" className="h-auto justify-start rounded-3xl p-4 text-left">
      <Link to={href}>
        <Icon className="mr-3 size-5 shrink-0 text-primary" />
        <span>
