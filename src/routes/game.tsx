        ...
        <div className="grid gap-3 lg:grid-cols-3">
          {quests.map((quest) => {
            ...
            return (
              <article
                key={quest.title}
                className={`rounded-3xl border p-5 transition-all ${quest.done ? "border-primary/50 bg-primary/10" : "border-border bg-card hover:border-primary/30"}`}
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <Badge variant={quest.done ? "default" : "outline"}>
                    {quest.done ? "CLEARED" : quest.reward ? `+${quest.reward} XP` : "MISSION"}
                  </Badge>
                </div>
                <h3 className="mt-4 text-lg font-bold">{quest.title}</h3>
                <p className="mt-1 text-sm text-muted-foreground">{quest.detail}</p>
                <Button
                  asChild
                  variant={quest.done ? "secondary" : "default"}
                  className="mt-5 w-full rounded-full"
                >
                  <Link to={quest.href}>{quest.done ? "Review" : "Start mission"}</Link>
                </Button>
              </article>
            );
          })}
      ...
      <div className="mt-7 flex items-center gap-3 rounded-3xl border border-border bg-muted/30 p-5">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            The UI is ready for future leaderboards, teams and challenges without pretending those
            features exist yet.
          </p>
        </div>
      </div>
