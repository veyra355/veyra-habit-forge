...
function HomePage() {
  ...
  const levelProgress = Math.max(0, Math.min(100, ((state.totalXp - previousLevelXp) / levelRange) * 100));
    ...
    <>
        ...
        <div className="relative grid gap-6 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
          <div>
            ...
            <div className="mt-3 flex items-end gap-3">
              ...
              <span className="pb-1 text-sm font-semibold text-muted-foreground">{state.currentRank}</span>
            </div>
      ...
      <section className="mt-6">
        <div className="mb-3 flex items-end justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-primary">Daily quests</p>
            <h2 className="mt-1 text-lg font-semibold">Build your character, one mission at a time.</h2>
          </div>
        ...
        <div className="grid gap-3 md:grid-cols-3">
          {QUESTS.map((quest) => {
            ...
            const done = quest.id === "workout" ? today.includes("workout") : quest.id === "habits" ? completedToday >= 3 : state.currentStreak > 0;
            return (
              <div key={quest.id} className={`panel p-4 transition-all ${done ? "border-primary/50 bg-primary/5" : ""}`}>
                <div className="flex items-start justify-between gap-3">
                  <div className="flex size-10 items-center justify-center rounded-2xl bg-secondary text-primary">
                    <Icon className="size-5" />
                  </div>
                  <Badge variant={done ? "default" : "outline"}>{done ? "Cleared" : quest.reward}</Badge>
                </div>
                <h3 className="mt-4 font-semibold">{quest.title}</h3>
                <p className="mt-1 text-xs text-muted-foreground">{done ? "Quest complete. Keep your momentum." : `Reward ${quest.reward}`}</p>
              </div>
            );
          })}
        </div>
            ...
            {state.habits.slice(0, 6).map((habit) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-xs text-muted-foreground">{done ? "Done · XP earned" : "Tap · earn XP"}</span>
                </button>
              );
            })}
          </div>
          ...
          <div className="mt-4 space-y-4 text-sm">
            <div>
              ...
              <ul className="mt-1.5 space-y-1 text-muted-foreground">
                {groomingRoutines.morning.slice(0, 3).map((i) => <li key={i}>· {i}</li>)}
              </ul>
            </div>
            <div>
              <p className="font-medium">Evening routine</p>
              <ul className="mt-1.5 space-y-1 text-muted-foreground">
                {groomingRoutines.evening.slice(0, 3).map((i) => <li key={i}>· {i}</li>)}
              </ul>
            </div>
          </div>
      ...
      <div className="panel mt-4 p-5">
        ...
        <div className="mt-4 space-y-2.5">
          {week.daily.map((d) => (
            <div key={d.date}>
              <div className="mb-1 flex justify-between text-xs"><span>{d.label}</span><span className="text-muted-foreground">{d.pct}%</span></div>
      ...
      <SafetyNote>
        Veyra offers general wellness and fitness guidance only. It does not diagnose conditions or treat injuries.
        For symptoms, pain, medication or medical questions, please consult a qualified healthcare professional.
      </SafetyNote>
    </>
  );
}

function GameStat({ icon: Icon, label, value }: { icon: typeof Flame; label: string; value: string }) {
  return (
