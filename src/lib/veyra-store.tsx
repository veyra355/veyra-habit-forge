...
function streaks(completions: Record<string, string[]>) {
  ...
  for (let i = 0; i < 365; i += 1) {
    if (!(completions[dayKey(i)] ?? []).length) break;
    current += 1;
  }
  let longest = 0;
  let run = 0;
  for (let i = 364; i >= 0; i -= 1) {
    if ((completions[dayKey(i)] ?? []).length) {
      run += 1;
...
function withProgress(
  ...
  return {
    ...
    longestStreak: Math.max(state.longestStreak, s.longest),
  };
}
...
async function loadProfile(
  session: Session,
): Promise<{ user: VeyraUser; onboarding: Onboarding | null }> {
  ...
  try {
    const { data } = await supabase
      .from("profiles")
      .select("display_name, email, plan, onboarding, created_at")
    ...
    return {
      user: {
        ...
        plan: (data?.plan as VeyraUser["plan"]) || "free",
        joinedAt: data?.created_at || session.user.created_at || new Date().toISOString(),
      },
...
async function loadRemoteState(userId: string) {
  const [p, h, c, w] = await Promise.all([
    supabase
      .from("user_progress")
      .select("total_xp, level, rank, current_streak, longest_streak")
      .eq("user_id", userId)
      .maybeSingle(),
    supabase
      .from("habits")
      .select("id, key, name, is_custom, sort_order")
      .eq("user_id", userId)
      .order("sort_order"),
    supabase
      .from("habit_completions")
      .select("habit_id, completed_on, habits!inner(key)")
      .eq("user_id", userId)
      .order("completed_on", { ascending: false })
      .limit(1000),
    supabase
      .from("workout_sessions")
      .select("id, performed_on, title, feedback")
      .eq("user_id", userId)
      .order("performed_on", { ascending: false })
      .limit(100),
  ]);
  const habits: HabitDefinition[] = h.data?.length
    ? h.data.map((x) => ({ id: x.key, name: x.name, custom: x.is_custom }))
    : DEFAULT_HABITS;
  const completions: Record<string, string[]> = {};
  for (const row of c.data ?? []) {
    const rel = Array.isArray(row.habits) ? row.habits[0] : row.habits;
    if (rel?.key)
      completions[row.completed_on] = [...(completions[row.completed_on] ?? []), rel.key];
  }
  const sessions = (w.data ?? []).map((x) => ({
    id: x.id,
    date: x.performed_on,
    title: x.title,
    feedback: (x.feedback as WorkoutSession["feedback"]) ?? null,
  }));
  return {
    totalXp: p.data?.total_xp ?? 0,
    currentLevel: p.data?.level ?? 1,
    currentRank: p.data?.rank
      ? p.data.rank.charAt(0).toUpperCase() + p.data.rank.slice(1)
      : "Bronze",
    currentStreak: p.data?.current_streak ?? 0,
    longestStreak: p.data?.longest_streak ?? 0,
    habits,
    completions,
    sessions,
  };
...
export function VeyraProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) setState((p) => ({ ...p, ...(JSON.parse(raw) as Partial<VeyraState>) }));
    } catch {
      /* ignore */
    }
    setHydrated(true);
  ...
  useEffect(() => {
    if (hydrated) document.documentElement.classList.toggle("dark", state.theme === "dark");
  ...
  useEffect(() => {
    ...
    const { data: listener } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      } else void hydrateSession(session).finally(() => setAuthLoading(false));
    });
    return () => {
      cancelled = true;
  ...
  const signUpWithPassword = useCallback(
    async (name: string, email: string, password: string): Promise<AuthResult> => {
      ...
      return data.session ? { error: null } : { error: null, needsEmailConfirmation: true };
    },
    [],
  );
  ...
  const toggleHabit = useCallback(
    (id: string, date = todayKey()) => {
      if (!state.user) return;
      const done = state.completions[date]?.includes(id) ?? false;
      setState((p) => {
        const list = p.completions[date] ?? [];
        const next = done ? list.filter((x) => x !== id) : [...list, id];
        return withProgress(p, p.totalXp + (HABIT_XP[id] ?? 20) * (done ? -1 : 1), {
          ...p.completions,
          [date]: next,
        });
      });
      void currentUserId().then(async (userId) => {
        ...
        if (done)
          ...
          const { error } = await supabase
            .from("habit_completions")
            .insert({ user_id: userId, habit_id: habit.id, completed_on: date });
          if (!error) await awardXp(HABIT_XP[id] ?? 20, "habit", `${id}:${date}`);
        }
      });
    },
  ...
  const addHabit = useCallback(
    (name: string) => {
      ...
      setState((p) => ({ ...p, habits: [...p.habits, { id: key, name, custom: true }] }));
  ...
  const removeHabit = useCallback(
    (id: string) => setState((p) => ({ ...p, habits: p.habits.filter((h) => h.id !== id) })),
    [],
  );
  const completeWorkout = useCallback(
    (title: string, feedback: WorkoutSession["feedback"]) => {
      setState((p) => ({
        ...p,
        sessions: [{ id: `local-${Date.now()}`, date: todayKey(), title, feedback }, ...p.sessions],
      }));
      void currentUserId().then(async (userId) => {
        if (!userId) return;
        const date = todayKey();
        const { error } = await supabase
          .from("workout_sessions")
          .insert({ user_id: userId, title, feedback, performed_on: date });
        if (!error) await awardXp(200, "workout", `workout:${date}`);
      });
    },
    [awardXp],
  );
    ...
    (id: string) =>
      setState((p) => ({
        ...
        completedExercises: p.completedExercises.includes(id)
          ? p.completedExercises.filter((e) => e !== id)
