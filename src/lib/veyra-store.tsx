...
export type Onboarding = { ageRange: string; experience: string; location: string; equipment: string[]; daysPerWeek: string; workoutTime: string; goals: string[]; sleepSchedule: string; activityLevel: string; workSchedule: string; timePerDay: string; diet: string; place: string };
...
export type VeyraState = { user: VeyraUser | null; onboarding: Onboarding | null; habits: HabitDefinition[]; completions: Record<string, string[]>; sessions: WorkoutSession[]; completedExercises: string[]; messages: ChatMessage[]; theme: "light" | "dark"; notifications: { workout: boolean; habits: boolean }; privacy: { shareProgress: boolean }; totalXp: number; currentLevel: number; currentRank: Rank; currentStreak: number; achievements: string[] };
...
const initialState: VeyraState = { user: null, onboarding: null, habits: [], completions: {}, sessions: [], completedExercises: [], messages: [], theme: "dark", notifications: { workout: true, habits: true }, privacy: { shareProgress: false }, totalXp: 0, currentLevel: 1, currentRank: "BRONZE", currentStreak: 0, achievements: [] };
type Result = { error?: string };
type Ctx = VeyraState & { state: VeyraState; hydrated: boolean; update: (patch: Partial<VeyraState>) => void; signIn: (email: string, password: string) => Promise<Result & { needsOnboarding?: boolean }>; signUp: (name: string, email: string, password: string) => Promise<Result>; signInWithGoogle: () => Promise<Result>; signOut: () => Promise<void>; toggleHabit: (id: string, date?: string) => Promise<void>; addHabit: (name: string) => Promise<void>; removeHabit: (id: string) => Promise<void>; completeWorkout: (title: string, feedback: WorkoutSession["feedback"]) => Promise<void>; toggleExercise: (id: string) => void; awardXp: (amount: number, nextCompletions?: Record<string, string[]>) => Promise<void> };
...
function toHabit(row: HabitRow): HabitDefinition {
  return { id: row.id, name: row.name, custom: !!row.custom };
}
...
interface ProfileRow {
  ...
  notifications?: { workout: boolean; habits: boolean };
  privacy?: { shareProgress: boolean };
  created_at?: string;
  total_xp?: number;
...
async function loadUser(userId: string, fallbackEmail = "") {
  ...
  (completionsRes.data ?? []).forEach((c: CompletionRow) => { (grouped[c.completed_on] ??= []).push(c.habit_id); });
  ...
  return {
    user: { id: userId, name: profileRes.data?.name || fallbackEmail.split("@")[0] || "Friend", email: profileRes.data?.email || fallbackEmail, role: profileRes.data?.role === "admin" ? "admin" : "user", plan: (profileRes.data?.plan ?? "free") as "free" | "starter" | "pro" | "elite", joinedAt: profileRes.data?.created_at || new Date().toISOString() },
    onboarding: (profileRes.data?.onboarding as Onboarding | null) ?? null,
    habits: (habitsRes.data ?? []).map(toHabit), completions: grouped,
    sessions: (sessionsRes.data ?? []).map((s: SessionRow) => ({ id: s.id, date: s.session_date, title: s.workout_title, feedback: s.feedback })),
...
export function VeyraProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    let mounted = true;
    db.auth.getSession().then(async ({ data, error }: { data: { session: { user: { id: string; email: string } } | null }; error: Error | null }) => { if (error) console.error("[Veyra] session restore failed", error); if (mounted && data.session) { try { await applyUser(data.session.user.id, data.session.user.email); } catch (e) { console.error("[Veyra] load user failed", e); } } setHydrated(true); });
    const { data: listener } = db.auth.onAuthStateChange((_event: string, session: { user: { id: string; email: string } } | null) => { setTimeout(() => { if (!mounted) return; if (session) void applyUser(session.user.id, session.user.email); else setState(initialState); }, 0); });
  ...
  const signIn = useCallback(async (email: string, password: string) => { const { data, error } = await db.auth.signInWithPassword({ email: email.trim(), password }); if (error) return { error: error.message }; const needsOnboarding = !data.user.user_metadata?.onboarding_complete; if (data.session) await applyUser(data.user.id, data.user.email ?? email); return { needsOnboarding }; }, [applyUser]);
  const signUp = useCallback(async (name: string, email: string, password: string) => { const { data, error } = await db.auth.signUp({ email: email.trim(), password, options: { data: { name: name.trim() } } }); if (error) return { error: error.message }; if (data.user) await applyUser(data.user.id, email); return {}; }, [applyUser]);
  ...
  const addHabit = useCallback(async (name: string) => { if (!state.user || !name.trim()) return; const { data, error } = await db.from("habits").insert({ user_id: state.user.id, name: name.trim() }).select(); if (error) throw new Error(error.message); if (data && data[0]) setState(prev => ({ ...prev, habits: [...prev.habits, toHabit(data[0])] })); }, [state.user]);
  const removeHabit = useCallback(async (id: string) => { if (!state.user) return; const { error } = await db.from("habits").delete().eq("id", id).eq("user_id", state.user.id); if (error) throw new Error(error.message); setState(prev => ({ ...prev, habits: prev.habits.filter(h => h.id !== id) })); }, [state.user]);
  const completeWorkout = useCallback(async (title: string, feedback: WorkoutSession["feedback"]) => { if (!state.user) return; const { data, error } = await db.from("workout_sessions").insert({ user_id: state.user.id, workout_title: title, session_date: todayKey(), feedback }).select(); if (error) throw new Error(error.message); if (data && data[0]) setState(prev => ({ ...prev, sessions: [{ id: data[0].id, date: data[0].session_date, title: data[0].workout_title, feedback: data[0].feedback }, ...prev.sessions] })); }, [state.user]);
  ...
  const value = useMemo(() => ({ ...state, state, hydrated, update, signIn, signUp, signInWithGoogle, signOut, toggleHabit, addHabit, removeHabit, completeWorkout, toggleExercise, awardXp }), [state, hydrated, update, signIn, signUp, signInWithGoogle, signOut, toggleHabit, addHabit, removeHabit, completeWorkout, toggleExercise, awardXp]);
